...
<?php
	foreach (scandir("sessions/") as $file) {
		if (preg_match("/(.*)-(.*)/", base64_decode($file), $m)) {
			...
			print("<a class=\"btn btn-default\" href=\"export.php?type=single&folder=sessions&session={$m[1]}-{$m[2]}&download=1\"><span class=\"glyphicon glyphicon-download-alt\"></span> download</a>\n");
...
<?php
	foreach (scandir("finished/") as $file) {
		if (preg_match("/(.*)-(.*)/", base64_decode($file), $m)) {
			...
			print("<a class=\"btn btn-default\" href=\"export.php?type=single&folder=finished&session={$m[1]}-{$m[2]}&download=1\"><span class=\"glyphicon glyphicon-download-alt\"></span> download</a>\n");
			print("<a class=\"btn btn-default\" href=\"export.php?type=dot&folder=finished&session={$m[1]}-{$m[2]}&download=1\"><span class=\"glyphicon glyphicon-picture\"></span> dot</a>\n");
